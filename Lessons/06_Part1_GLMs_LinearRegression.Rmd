...
**if P-value<a, we reject the null hypothesis because we have enough reason to reject the null one for entire population.
**if P-value>=a, we can not reject null because we do not have enough evidence to reject the null hypothesis for entire population.

1.Standard error of estimate:a measure of the scatter of points around a regression line; smaller means more accurate.
2.Correlation r:a measure of the strength and direction of relationship between two variables, between -1~+1.
3.Square correlation:r2, a measure of the percentage of variability in the values of y that is explained by the independent variable. Range between 0 to 1.
4.P-value: the coefficients indicate whether these relationship are statistically significant. The null hypothesis for each independent variable:Ho independent variable has no correlation with the dependent variable.
...
#P is smaller than 0.05 and statically different than 0, it is worthwhile to try to estimate irradiance level based on the depth

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
#-0.555968 is the correlation
```
...
par(mfrow = c(2,2), mar=c(4,4,4,4))#divide the screen two by two
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #y axe will be log transformed
  geom_point() 
...
plot(TPregression)
par(mfrow = c(1,1))
#we look the symetry among the both of red line and lien should be flat
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) #correlation matrix among these five variables.
...
step(TPAIC)
#when you remove po4,the AIC is 352, higher is worse,compare the step AIC
