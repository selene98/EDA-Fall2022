...
print(ex7.1)
#seasonal trend, overall stable
ex7.2<-
  ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  geom_smooth(method = lm, color="pink")
print(ex7.2)
#seasonal trend and going up overall
...
BRA_complete.ts <- ts(BRA_complete$Date, start = c(1901,1), frequency = 12)
...
BRA.decomposed <- stl( BRA_complete.ts, s.window = "periodic")
plot(BRA.decomposed)
