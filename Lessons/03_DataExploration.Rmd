...
# Relative file path to store data in environment (friendly for users regardless of machine)
...
# Rename columns function c is used for creation
...
str(USGS.flow.data)
dim(USGS.flow.data) #short for dimension, how many lines and columns
length(USGS.flow.data)

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #only column 3,8 and 14, row from 30000 to 30005
...
summary(USGS.flow.data$datetime)
```
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #remove rows contain NA
dim(USGS.flow.data)
