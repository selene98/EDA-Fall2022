...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #default ggplot theme so no difference with the previous one
print(O3plot1)
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #gray lines and axes on white backgrounds
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #classic-looking theme, no gridlines background
print(O3plot4)
```
...
#the labels font is 14, legend on top of graph
...
NvsP2 <-
  ...
  labs(color="Depth", shape="Lakename") + # change your legend labels here
  scale_shape_manual(values = c(15, 17)) + #search fo the exact number
...
#adjust the area ont plot needs, like the upper size is 1.25
```
...
ggplot(Neonics) +
 ...
 # reorder(x = Neonics$Endpoint,FUN = length)+
