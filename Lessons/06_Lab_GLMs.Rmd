...
```{r}
#Exercise 1:
ex1 <- ggplot(EPAair, aes(x = Ozone, y =as.factor(Year))) +
  geom_density(alpha = 0.5)+
  scale_fill_manual(values = c("green", "blue"))+
  labs(x= "Ozone AQI quality")
print(ex1)

ex2 <- ggplot(EPAair,aes(y=Ozone, x = as.factor(Year)))+
  geom_boxplot()+
  stat_summary(geom = "text", fun = max, vjust = -1, size = 4,
               label=c("b", "a"))+
  labs(x="",y="Ozone AQI quality")+
  ylim(0,150)
print(ex2)

ex3 <- ggplot(EPAair,aes(y=Ozone, x = as.factor(Year)))+
...
aq.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(aq.regression)
