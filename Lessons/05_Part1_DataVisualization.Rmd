...
getwd()
library(tidyverse)
#install.packages("ggridges"); #look at the distribution of variables
library(ggridges)
...
* geom_bar; bar plot
* geom_histogram
* geom_freqpoly; frequence plot
* geom_boxplot
* geom_violin
* geom_dotplot
* geom_density_ridges
* geom_point
* geom_errorbar; trend line
* geom_smooth
* geom_line; h-line & v-line
* geom_area
...
* fill; change the bar or point color
* shape; decide the shape of point
* size; for point
* transparency, for points and bars
...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point() #the point is not consistent because we chose the geom
...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone, color = "blue")) #adding another layer rather than just changing color.
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? 
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year) , color = Site.Name)) + ##assign the different shape by year and different color by the site names
  geom_point()
print(PMplot)
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #adding columns for site names and year
...
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + #the number of x axe is from 0 to 250
  ylim(0, 20)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #because the depth is getting bigger when going down, we want it go from large number to 0
print(tempvsdepth)
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) #already know the line
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? color changes the point color by group and fill only change the color of the box
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  ylim(0, 2500)#stack the observation over y-axe
print(Nutrientplot4)

# Violin plot-also discribe the density of data
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Ridgeline plot-change fill not color, alpha means the transparency
Nutrientplot6 <-
