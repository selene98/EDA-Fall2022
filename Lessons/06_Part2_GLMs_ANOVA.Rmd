...
**if you reject the null hypothesis, it means at least one pair mean is not equal, but you do not know which two or more are not equal, at this time, the post-hoc test is applied to examine it.
...
# Test for normality. the population is suited for normal distribution
...
#results: reject null(follow the normal distribution) in all except two: NIWO_057 and NIWO_046
#but method is robust from small deviations from normal distribution, we can see from the plot it does not follow the normal distribution.
...
# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID) #continuour variable ~ categorial variable
...
#if the group could be compared, it means the difference between groups is not statistically significant
