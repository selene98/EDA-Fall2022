...
library(lubridate)
library(ggpmisc)
litter.chem.phy <- read.csv("../Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)
litter.chem.phy$sampledate <- as.Date(litter.chem.phy$sampledate , format = "%m/%d/%y")
...
#10
tem.regression <- lm(data = NTL.litter, temperature_C ~ year4 + daynum + depth)
...
>Answer:Central long lake,Tuesday lake, West long lake and East long lake have the same mean temperature with Peter lake. Hummingbird Lake have a mean temperature that is statistically distinct from all the other lakes.
